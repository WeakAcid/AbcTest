...
const int kDefaultMaxBitrateBps = 2000000;
...
void SendSideBandwidthEstimation::SetBitrates(int send_bitrate,
  ...
  if (send_bitrate > 0)
    SetSendBitrate(send_bitrate, 0);
}
//2020 mobicom
//change the number of inputs
void SendSideBandwidthEstimation::SetSendBitrate(int bitrate,int what_based) {
...
void SendSideBandwidthEstimation::CapBitrateToThresholds(int64_t now_ms,
                                                         uint32_t bitrate_bps,int is_RL_based) {
  
  int rtt = last_round_trip_time_ms_;
  float loss = last_fraction_loss_/256.0f;
  if(RL_flag  && is_RL_based)
  {
		
  }
  else if(!RL_flag && !is_RL_based)
  {
  ...
  if((RL_flag  && is_RL_based) || (!RL_flag  && !is_RL_based))
  ...
  std::cout<< pTime->tm_hour<<":"<<pTime->tm_min<<":"<<pTime->tm_sec<<":"<<tv.tv_usec/1000<<" "<<rtt<<" "<<loss<<" "<<current_bitrate_bps_<<" "<<bwe_incoming_<<" what_based"<< is_RL_based<<" RL_flag"<<RL_flag<<std::endl;
  std::ofstream out("rtt_loss_bitrate.txt",std::ios::app);
  out<<pTime->tm_hour<<":"<<pTime->tm_min<<":"<<pTime->tm_sec<<":"<<tv.tv_usec/1000<<" "<<rtt<<" "<<loss<<" "<<current_bitrate_bps_<<" "<<bwe_incoming_<<" what_based"<< is_RL_based<<" RL_flag"<<RL_flag<<std::endl;
  out.close();
}
}  // namespace webrtc
